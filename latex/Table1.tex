...
\caption{The 13 pulsars of this proposal. The pulsar parameters DM, $P_0$, $W_{50}$, $S_{1400}$, and $V_{\rm trans}$ are taken from the ATNF pulsar catalogue. The on-pulse flux density is calculated as $S_{W50}=S_{1400}P_0/W_{50}$. $T_{B{\rm max}}$ is the maximum \hi~brightness temperature along each sightline, taken from the HI-4pi urvey. $S_{\rm RMS}$ is calculated for $N_{\rm antenna}=26$, $T_{\rm on}=1$~hr, and $\Delta\nu=9.600$~kHz, and $\tau_{5\sigma}=5S_{\rm RMS}/S_{W50}$. $D_{3{\rm months}}$ is calculated from $V_{\rm trans}$ for a 3-month interval, using 30 days per month.}
\end{center}
\end{table}
...
%{\rm and}\ \ \ \tau_{5\sigma} &=& 5 \times \frac{S_{\rm RMS}}{S_{W50}} ,
%\end{eqnarray}
