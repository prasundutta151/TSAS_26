...
For an ATNF best-distance sample, use `csv/sample_pulsar_ATNF.csv`. It uses ATNF values for the catalogue columns, recomputes `SW50` from ATNF `S1400`, `P0`, and `W50`, carries over `RMS`, `t5sigma`, and `Ton`, and removes `D (21) AU` and `D (22) AU`. Its `DIST kpc` and `V_trans km/s` values are marked with `*` when the distance is not a VLBI/parallax distance.
...
The calculation script accepts starred numeric values such as `1.600*` and `361.330*`. When a starred `V_trans` value is used for `D(3mths) AU` or `D(6mths) AU`, the output distance also carries `*`.
